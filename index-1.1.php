...
function getSiteName(){
    ...
    return $cfg['site_name'] ?? '尚显SOP展示系统';
}
...
function initData()
{
    ...
    if (!file_exists(USER_FILE)) {
        saveJson(USER_FILE, [
            ['id'=>1,'username'=>'admin','pwd'=>password_hash('123456',PASSWORD_DEFAULT),'name'=>'管理员','role'=>'admin']
        ]);
    }
...
function login(){
    ...
    .then(res=>res.json()).then(ret=>{ret.code?alert(ret.msg):location.href='?act=home';})
}
</script>
</body>
</html>
...
<div style="background:#242933;padding:0 20px;display:flex;align-items:center;justify-content:space-between;color:#fff;line-height:50px;flex-wrap:wrap;position:sticky;top:0;z-index:99;">
    ...
    <div>
        <a href="?act=home" style="color:#fff;margin:0 8px;text-decoration:none;background:#2d8cf0;padding:6px 12px;border-radius:4px;">🏠 返回首页</a>
        <?php if(isAdmin()):?>
        ...
        <?php else:?>
        <a href="?act=login" style="color:#fff;margin:0 8px;text-decoration:none;background:#666;padding:6px 12px;border-radius:4px;">管理登录</a>
        <?php endif;?>
    </div>
</div>
<?php
...
.go-top-right{position:fixed;right:15px;bottom:80px;background:#242933;color:#fff;width:45px;height:45px;text-align:center;line-height:45px;border-radius:50%;cursor:pointer;z-index:999;}
...
<div class="go-top-right" onclick="window.scrollTo({top:0,behavior:'smooth'})">↑</div>
<?php
}
...
?>
<div style="width:240px;background:#fff;border-right:1px solid #eee;min-height:calc(100vh - 50px);padding:15px;position:fixed;left:0;top:50px;">
    ...
    <div style="background:#fff;padding:20px;border-radius:8px;">
        ...
        <form method="post" action="?act=save_config">
            <div style="margin:15px 0;">
                <label>系统标题</label>
                <input type="text" name="site_name" value="<?=e($cfg['site_name'])?>" required style="width:100%;padding:10px;border:1px solid #eee;border-radius:4px;">
            </div>
            <button type="submit" style="padding:8px 20px;background:#2d8cf0;color:#fff;border:none;border-radius:4px;">保存修改</button>
        </form>
    </div>
</div>
<?php
...
function homePage(){
    ...
    if($c2 <= 0){
        $afterSaleIds = array_column(getCate2ByPid(2),'id');
        $temp = [];
        foreach($sopList as $v){
            if(in_array($v['cate2'],$afterSaleIds)) $temp[]=$v;
        }
        $sopList = $temp;
    }else{
        $temp=[];
        foreach($sopList as $v){
            if($v['cate2']==$c2) $temp[]=$v;
        }
        $sopList = $temp;
    }
?>
...
<div class="main-wrap">
    <div style="padding:20px;max-width:1200px;margin:0 auto;">
        <div style="background:#e8f4ff;padding:12px;border-radius:6px;margin-bottom:20px;">
            <strong>📌 默认展示：售后部门SOP</strong>
            <?php if($c2>0):?>
            <span style="margin-left:15px;">当前岗位：<?=getCate2Name($c2)?></span>
            <a href="?act=home" style="margin-left:15px;color:#2d8cf0;">重置筛选</a>
            <?php endif;?>
        </div>
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(340px,1fr));gap:15px;">
        <?php if(empty($sopList)):?>
            <div style="padding:60px 0;text-align:center;color:#999;">暂无SOP文档</div>
        <?php else:?>
            <?php foreach($sopList as $v):?>
            <div style="background:#fff;padding:16px;border-radius:8px;box-shadow:0 2px 6px rgba(0,0,0,0.05);">
                ...
                <div style="font-size:13px;color:#666;margin-bottom:10px;">
                    <?=getCate1Name($v['cate1'])?> / <?=getCate2Name($v['cate2'])?>
                </div>
                <div style="max-height:120px;overflow:hidden;font-size:14px;line-height:1.8;">
                    <?=$v['content']?>
                </div>
                <div style="margin-top:12px;display:flex;gap:10px;">
                    ...
                    <?php if(isAdmin()):?>
                    <a href="?act=sop_edit&id=<?=$v['id']?>" style="padding:5px 12px;background:#28a745;color:#fff;border-radius:4px;text-decoration:none;font-size:13px;">编辑</a>
                    <?php endif;?>
                </div>
            </div>
            <?php endforeach;?>
...
function viewSop(){
    ...
    if(empty($info)){echo '<div style="padding:30px;text-align:center;">文档不存在 <a href="?act=home">返回首页</a></div>';exit;}
?>
<div style="padding:20px;max-width:900px;margin:0 auto;">
    <div style="background:#fff;padding:30px;border-radius:8px;">
        <h2 style="text-align:center;margin:0 0 20px;"><?=e($info['title'])?></h2>
        <div style="text-align:center;color:#666;margin-bottom:20px;">
            所属部门：<?=getCate1Name($info['cate1'])?>｜岗位：<?=getCate2Name($info['cate2'])?>
            <?php if(isAdmin()):?>
                <a href="?act=sop_edit&id=<?=$info['id']?>" style="margin-left:20px;color:#28a745;">点击编辑此SOP</a>
            <?php endif;?>
        </div>
        <div style="line-height:2;padding:25px;border:1px solid #eee;border-radius:6px;">
            <?=$info['content']?>
        </div>
        <div style="margin-top:25px;text-align:center;">
            <a href="?act=home" style="padding:8px 25px;background:#2d8cf0;color:#fff;border-radius:4px;text-decoration:none;">返回首页</a>
        </div>
    </div>
</div>
<?php
...
<div style="padding:20px;max-width:1000px;margin:0 auto;">
    <div style="background:#fff;padding:20px;border-radius:8px;">
        ...
        <form method="post" action="?act=sop_save">
            ...
            <div style="margin:15px 0;">
                <label>所属部门</label>
                <select name="cate1" id="cate1" onchange="loadCate2()" style="width:100%;padding:8px;margin-top:5px;">
            ...
            <div style="margin:15px 0;">
                <label>所属岗位</label>
                <select name="cate2" id="cate2" style="width:100%;padding:8px;margin-top:5px;">
                    <?php foreach(getCate2ByPid($cate1[0]['id']) as $v):?>
                    <option value="<?=$v['id']?>"><?=e($v['name'])?></option>
                    <?php endforeach;?>
                </select>
            </div>
            <div style="margin:15px 0;">
                <label>SOP标题</label>
                <input type="text" name="title" required style="width:100%;padding:10px;border:1px solid #eee;border-radius:4px;margin-top:5px;">
            </div>
            <div style="margin:15px 0;">
                <label>文本内容编辑（选中文字点按钮改颜色/加粗）</label>
                <div class="tool-bar">
                    <button type="button" onclick="formatBold()"><b>B</b></button>
                    <button type="button" onclick="formatFontColor('#ff0000')" class="color-btn" style="background:#ff0000;" title="红色"></button>
                    <button type="button" onclick="formatFontColor('#ff6600')" class="color-btn" style="background:#ff6600;" title="橙色"></button>
                    <button type="button" onclick="formatFontColor('#cccc00')" class="color-btn" style="background:#cccc00;" title="黄色"></button>
                    <button type="button" onclick="formatFontColor('#009900')" class="color-btn" style="background:#009900;" title="绿色"></button>
                    <button type="button" onclick="formatFontColor('#0066ff')" class="color-btn" style="background:#0066ff;" title="蓝色"></button>
                    <button type="button" onclick="formatFontColor('#9933cc')" class="color-btn" style="background:#9933cc;" title="紫色"></button>
                    <button type="button" onclick="formatFontColor('#333333')" class="color-btn" style="background:#333333;" title="黑色"></button>
            ...
            <button type="submit" style="padding:10px 30px;background:#2d8cf0;color:#fff;border:none;border-radius:4px;">提交保存</button>
        </form>
    </div>
</div>
<script>
    ...
    <div style="background:#fff;padding:20px;border-radius:8px;">
        ...
        <form method="post" action="?act=sop_save">
            ...
            <div style="margin:15px 0;">
                ...
                <select name="cate1" id="cate1" onchange="loadCate2()" style="width:100%;padding:8px;margin-top:5px;">
                    <?php foreach($cate1 as $v):?>
                    <option value="<?=$v['id']?>" <?=$info['cate1']==$v['id']?'selected':''?>><?=e($v['name'])?></option>
                    <?php endforeach;?>
                </select>
            </div>
            <div style="margin:15px 0;">
                <label>所属岗位</label>
                <select name="cate2" id="cate2" style="width:100%;padding:8px;margin-top:5px;">
                    <?php foreach(getCate2ByPid($info['cate1']) as $v):?>
                    <option value="<?=$v['id']?>" <?=$info['cate2']==$v['id']?'selected':''?>><?=e($v['name'])?></option>
                    <?php endforeach;?>
                </select>
            </div>
            <div style="margin:15px 0;">
                <label>SOP标题</label>
                <input type="text" name="title" value="<?=e($info['title'])?>" required style="width:100%;padding:10px;border:1px solid #eee;border-radius:4px;margin-top:5px;">
            </div>
            <div style="margin:15px 0;">
                <label>文本内容编辑（选中文字点按钮改颜色/加粗）</label>
                <div class="tool-bar">
...
<div style="display:flex;justify-content:space-between;"><h3>一级部门</h3><a href="?act=cate1_add" style="background:#2d8cf0;color:#fff;padding:6px 12px;border-radius:4px;text-decoration:none;">新增</a></div>
<table width="100%" border="1" cellpadding="10" style="border-collapse:collapse;margin-top:15px;">
<?php foreach($list as $v):?>
<tr><td><?=$v['id']?></td><td><?=e($v['name'])?></td>
<td><a href="?act=cate1_edit&id=<?=$v['id']?>">编辑</a> <a href="?act=cate1_del&id=<?=$v['id']?>" onclick="return confirm('确定？')">删除</a></td></tr>
<?php endforeach;?>
</table></div></div><?php }
...
<form method="post" action="?act=cate1_save"><input type="hidden" name="id" value="0">
...
if($id==0){$newId=$list?max(array_column($list,'id'))+1:1;$list[]=['id'=>$newId,'name'=>$name,'sort'=>10];}
...
<div style="display:flex;justify-content:space-between;"><h3>二级岗位</h3><a href="?act=cate2_add" style="background:#2d8cf0;color:#fff;padding:6px 12px;border-radius:4px;text-decoration:none;">新增</a></div>
<table width="100%" border="1" cellpadding="10" style="border-collapse:collapse;margin-top:15px;">
<?php foreach($list as $v):?>
...
if($id==0){$newId=$list?max(array_column($list,'id'))+1:1;$list[]=['id'=>$newId,'pid'=>$pid,'name'=>$name,'sort'=>10];}
...
function sopList(){if(!isAdmin())exit;adminNav();floatTopBtn();$list=readJson(SOP_FILE);?>
<div style="padding:20px;max-width:1100px;margin:0 auto;"><div style="background:#fff;padding:20px;border-radius:8px;">
<div style="display:flex;justify-content:space-between;"><h3>SOP管理</h3><a href="?act=sop_add" style="background:#2d8cf0;color:#fff;padding:6px 12px;border-radius:4px;text-decoration:none;">新增SOP</a></div>
<table width="100%" border="1" cellpadding="10" style="border-collapse:collapse;margin-top:15px;">
<?php foreach($list as $v):?>
...
function sopDel(){if(!isAdmin())exit;$id=(int)$_GET['id'];$list=readJson(SOP_FILE);foreach($list as $k=>$v)if($v['id']==$id)unset($list[$k]);saveJson(SOP_FILE,array_values($list));header('Location:?act=sop_list');exit;}
